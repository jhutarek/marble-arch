...
sealed class NavigationUseCase<in I, out O> : UseCase<I, O> {
    ...
    @Singleton
    class NavigateTo @Inject constructor(private val controller: NavigationController) : NavigationUseCase<Int, Unit>() {
        override fun invoke(input: Int) = controller.navigateTo(input)
    }
}