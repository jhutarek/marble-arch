...
abstract class MarbleFragment<M : MarbleViewModel<S>, S : MarbleState> : Fragment() {
    ...
    protected fun Observable<S>.subscribeWithViewUpdatesDisabled(onValue: (S) -> Unit): Disposable = subscribe {
        isUpdatingViewRelay.accept(true)
        onValue(it)
        isUpdatingViewRelay.accept(false)
    }

    protected fun <T> Observable<T>.whenNotUpdatingView(): Observable<T> =
